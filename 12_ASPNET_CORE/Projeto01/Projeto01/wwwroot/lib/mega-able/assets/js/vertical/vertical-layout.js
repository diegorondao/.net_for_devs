...
$(document).ready(function() {
    ...
    function checkCookie() {
        var temp = getCookie("NavbarBackground");
        if (temp != "") {
            Navbarbg = temp;
        }
        var temp = getCookie("header-theme");
        if (temp != "") {
            headerbg = temp;
        }
        var temp = getCookie("menu-title-theme");
        if (temp != "") {
            menucaption = temp;
        }
        var temp = getCookie("themebg-pattern");
        if (temp != "") {
            bgpattern = temp;
        }
        var temp = getCookie("active-item-theme");
        if (temp != "") {
            activeitemtheme = temp;
        }
        var temp = getCookie("fream-type");
        if (temp != "") {
            frametype = temp;
        }
        var temp = getCookie("layoutlayout");
        if (temp != "") {
            layout_type = temp;
        }
        var temp = getCookie("sidebar-img");
        if (temp != "") {
            nav_image = temp;
        }
        var temp = getCookie("sidebar-img-type");
        if (temp != "") {
            active_image = temp;
        }
        var temp = getCookie("vertical-layout");
        if (temp != "") {
            layout_width = temp;
        }
        var temp = getCookie("vertical-effect");
        if (temp != "") {
            menu_effect_desktop = temp;
        }
    }
    checkCookie();
    $("#pcoded").pcodedmenu({
    ...
    function handlelayouttheme() {
        $('.theme-color > a.Layout-type').on("click", function() {
            ...
            if (layout == 'reset') {
                ...
                setCookie("sidebar-img", null, 0);
                setCookie("sidebar-img-type", null, 0);
